...
/**
 * Custom hook to manage the prediction form state and logic
 */
export function usePredictionForm() {
  ...
  const validFormValues = useMemo(() => {
    try {
      ...
      if (
        ...
        !valuesToValidate.weight ||
        !valuesToValidate.height ||
  ...
  const realtimePrediction = useRealtimePrediction(
    validFormValues as UserInputData,
  ...
  const onSubmit = (data: UserInputData) => {
